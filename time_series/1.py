...
df.dropna(subset=['odo'], how='all', inplace=True)

# Convert timestamp in to datetime format
...
IF.fit(model)
# New Outliers Column
df['Outliers'] = pd.Series(IF.predict(model)).apply(lambda x: 1 if x == -1 else 0)
# Get Anomaly Score
score = IF.decision_function(model)
